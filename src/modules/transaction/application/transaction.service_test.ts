...
import { MockTransactionDetailRepository } from "../__tests__/mocks/transaction-detail.repository.mock.ts";
import {
  createTransactionData,
  transactionsList,
  updateTransactionData,
} from "../__tests__/fixtures/transaction.fixtures.ts";
...
Deno.test("TransactionService - getMany delegates to repository", async () => {
  ...
  const mockDetailRepo = new MockTransactionDetailRepository();
  const service = new TransactionService(mockRepo, mockDetailRepo);
...
Deno.test("TransactionService - create delegates to repository", async () => {
  ...
  assertEquals(calls.length, 1);
  assertEquals(result.number, createTransactionData.number);
});
...
Deno.test("TransactionService - update delegates to repository", async () => {
  ...
  assertEquals(calls.length, 1);
  assertEquals(result.status, updateTransactionData.status);
});
...
Deno.test("TransactionService - getOne propagates repository errors", async () => {
  const mockRepo = new MockTransactionRepository({
    shouldThrow: new Error("Transaction not found"),
  });
  const mockDetailRepo = new MockTransactionDetailRepository();
  const service = new TransactionService(mockRepo, mockDetailRepo);
...
Deno.test("TransactionService - create propagates repository errors", async () => {
  ...
  await assertRejects(
    () => service.create(createTransactionData),
    Error,
    "Validation failed",
  );
});
...
Deno.test("TransactionService - update propagates repository errors", async () => {
  ...
  await assertRejects(
    () => service.update(999, updateTransactionData),
    Error,
    "Transaction not found",
  );
});
