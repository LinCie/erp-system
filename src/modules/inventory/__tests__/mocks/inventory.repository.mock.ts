...
class MockInventoryRepository implements IInventoryRepository {
  ...
  async getMany(props: GetManyInventoriesProps): Promise<GetManyInventoriesReturn> {
  ...
  async create(data: Omit<InventoryEntity, "id">): Promise<InventoryEntity> {
    ...
    if (this.shouldThrow) throw this.shouldThrow;
    const newId = this.inventories.length > 0 ? Math.max(...this.inventories.map((i) => i.id)) + 1 : 1;
    const newItem: InventoryEntity = { ...data, id: newId, created_at: new Date(), updated_at: new Date() };
    this.inventories.push(newItem);
    return newItem;
  }

  async update(id: number, data: Partial<InventoryEntity>): Promise<InventoryEntity> {
    ...
    const updated: InventoryEntity = { ...this.inventories[index], ...data, id, updated_at: new Date() };
  ...
  async delete(id: number): Promise<void> {
    ...
    const index = this.inventories.findIndex((i) => i.id === id);
    if (index === -1) throw new Error("Inventory not found");
    this.inventories[index] = { ...this.inventories[index], status: "archived", deleted_at: new Date() };
  }
}
